...
async def preprocess_query(query: str) -> int:
    query = query.strip()  # Удаляем лишние пробелы

    # 1. Проверка на "чистый" артикул (только цифры)
    if re.fullmatch(r'^\d+$', query):
        return int(query)

    # 2. Проверка на URL Wildberries (в любом месте строки, даже с параметрами)
    url_match = re.search(
        r'(?:https?://)?(?:www\.)?wildberries\.ru/catalog/(\d+)',
        query,
        flags=re.IGNORECASE  # Игнорируем регистр (на случай WildBerries)
    )
    if url_match:
        return int(url_match.group(1))

    # 3. Если ни один вариант не подошел — ошибка
...
async def get_product_response(session: ClientSession, article: int) -> str:
    request_ulr: str = "https://www.wildberries.ru/__internal/card/cards/v4/detail"
    params = {
        "appType": 1,
        "curr": "rub",
        "dest": -1257786,
